export const navItems = [
    { name: "About", link: "#about" },
    { name: "Projects", link: "#projects" },
    { name: "Contact ", link: "#contact" },
];

export const gridItems = [
    {
        id: 1,
        title: "Tech stack",
        description: "this is my",
        className: "md:col-span-5 md:row-span-7",
        imgClassName: "",
        titleClassName: "justify-center",
        img: "",
        spareImg: "",
    },
    {
        id: 2,
        title: "What I listen to.",
        description: "this is",
        className: "lg:col-span-2 md:col-span-3 md:row-span-1",
        titleClassName: "justify-start",
        img: "./spotify.svg",
        imgClassName: "absolute right-4 bottom-4 w-16 h-16",
    },

    {
        id: 3,
        title: "Pride",
        description: "my",
        className: "md:col-span-3 md:row-span-2 sm:row-span-3",
        imgClassName: "absolute right-0 bottom-0 w-50 h-40 sm:w-58 sm:h-48 md:w-74 md:h-64 lg:w-82 lg:h-72 xl:w-90 xl:h-80 max-w-lg max-h-lg",        titleClassName: "mb-2",
        img: "./lions2.png",
        spareImg: "./vandy.png",
        spareImgClassName: "absolute left-16 sm:left-0 bottom-0 w-40 h-40 sm:w-48 sm:h-48 md:w-64 md:h-64 lg:w-72 lg:h-72 xl:w-80 xl:h-80 max-w-lg max-h-lg",},
    {
        id: 4,
        title: "Contact Me",
        description: "if you'd like to",
        className: "lg:col-span-2 md:col-span-3 md:row-span-1",
        imgClassName: "",
        titleClassName: "md:max-w-full max-w-60 ",
        img: "",
        spareImg: "",
    },
    // {
    //     id: 3,
    //     title: "Currently building an Interpreter in Go",
    //     description: "Slime",
    //     className: "md:col-span-3 md:row-span-2",
    //     imgClassName: "absolute right-0 bottom-0 md:w-96 w-60",
    //     titleClassName: "justify-center md:justify-start lg:justify-center",
    //     img: "",
    //     spareImg: "",
    // },

];

export const projects = [

];


export const companies = [
    {
        id: 1,
        name: "Vanderbilt University",
        img: "",
        nameImg: "",
    },


];

export const workExperience = [
    {
        id: 1,
        title: "Research Assistant",
        desc: "Research",
        className: "md:col-span-2",
        thumbnail: "",
    },
    {
        id: 2,
        title: "Teaching Assistant - Data Structures & Algorithms",
        desc: "TA.",
        className: "md:col-span-2", // change to md:col-span-2
        thumbnail: "",
    },

];

export const socialMedia = [
    {
        id: 1,
        img: "/git.svg",
    },
    {
        id: 2,
        img: "/twit.svg",
    },
    {
        id: 3,
        img: "/insta.svg",
    },
];